          ...
          <div className="space-y-3">
            {logs.map((log) => {
              ...
              <div key={log.id}
                ...
                style={{ border: '1px solid #E8E6E2', borderLeft: `4px solid ${sevColor.border}`, position: 'relative', overflow: 'hidden' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: '#fff', border: '1px solid #E8E6E2', borderRadius: '0.25rem',
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.75rem', paddingRight: '2.5rem' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontWeight: 600, color: '#111', fontSize: '0.9375rem', marginBottom: '0.25rem', wordBreak: 'break-word' as const, overflowWrap: 'anywhere' as const }}>{log.source}</p>
                    <p style={{ fontSize: '0.8125rem', color: '#555', wordBreak: 'break-word' as const, overflowWrap: 'anywhere' as const }}>{log.error_message}</p>
                  </div>
                  {!log.resolved && (
                    <button onClick={() => markResolved(log.id)}
                      disabled={resolvingId === log.id}
                      style={{
                        marginLeft: '1rem', padding: '0.375rem 0.75rem', borderRadius: '0.5rem',
                        ...
                        fontWeight: 500, fontSize: '0.8125rem', flexShrink: 0, whiteSpace: 'nowrap' as const
                      }}>
                ...
                {log.context && Object.keys(log.context).length > 0 && (
                  <div style={{ marginTop: '0.75rem', padding: '0.75rem', borderRadius: '0.5rem', background: '#FAFAF9', border: '1px solid #E8E6E2', overflow: 'hidden' }}>
                    <p style={{ fontSize: '0.6875rem', fontWeight: 500, color: '#888', marginBottom: '0.375rem' }}>Context:</p>
                    <pre style={{ fontSize: '0.6875rem', color: '#555', whiteSpace: 'pre-wrap' as const, wordBreak: 'break-all' as const, margin: 0 }}>{JSON.stringify(log.context, null, 2)}</pre>
                  </div>
                )}
                {log.error_stack && (
                  <details style={{ marginTop: '0.75rem' }}>
                    <summary style={{ fontSize: '0.6875rem', fontWeight: 500, color: '#888', cursor: 'pointer' }}>Stack Trace</summary>
                    <pre style={{ marginTop: '0.5rem', padding: '0.75rem', borderRadius: '0.5rem', background: '#FAFAF9', border: '1px solid #E8E6E2', fontSize: '0.6875rem', color: '#555', whiteSpace: 'pre-wrap' as const, wordBreak: 'break-all' as const, margin: '0.5rem 0 0 0' }}>{log.error_stack}</pre>
                  </details>
                )}
              </div>
              )
